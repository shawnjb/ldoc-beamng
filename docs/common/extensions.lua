...
local function luaPathToExtName(filepath)
  ...
  local res = filepath:gsub('_', '__'):gsub('/', '_')
  -- log('I', logTag, 'luaPathToExtName >> ' .. tostring(filepath) .. ' => ' .. tostring(res))
...
local function buildHookCache(funcName)
  local hookFuncs = {}
  ...
  if not tableIsEmpty(toResolveModules) then
    ...
    if not tableIsEmpty(failedModules) then
      ...
      for k, v in pairs(failedModules) do
        log('W', logTag, '    ' .. tostring(k) .. ' depencencies not resolved: '..dumps(v))
      end
    end
  end
...
local function printExtensions()
  ...
  for j, m in ipairs(resolvedModules) do
    log("I", "", " - "..dumps(m.__extensionName__))
  end
end
...
local function hookCount(funcName)
  luaExtensionFuncs[funcName] = luaExtensionFuncs[funcName] or buildHookCache(funcName)
  return #luaExtensionFuncs[funcName]
end
