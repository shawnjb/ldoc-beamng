...
--- Converts an RGB color vector to an HSV color vector.
--- @param rgb vec3 A vector representing the red, green, and blue components of the color (in the range [0,1]).
--- @return vec3 vector A vector representing the hue, saturation, and value (brightness) of the color (in the range [0,1]).
...
function addLevelLog(object, logLevel, origin, uniqueErrorCode, message)
	...
	if not object then
		Engine.Debug.addLevelLogLuaNoObject(logLevel, origin, uniqueErrorCode, debugInfo.source, debugInfo.currentline,
		message)
	else
		Engine.Debug.addLevelLogLua(object, logLevel, origin, uniqueErrorCode, debugInfo.source, debugInfo.currentline,
		message)
	end
end
...
function jsonWriteFile(filename, obj, pretty, numberPrecision)
	...
	if objType ~= "table" then
		log("E", "jsonWriteFile",
		"Provided objType is not a table but " .. dumps(objType) .. ", unable to write to: " .. dumps(filename))
...
function checkTableDataTypes(data, expectedTypes)
	...
	if #data ~= #expectedTypes then
		return false,
		string.format("Actual and expected parameter counts are mismatched, actual: %d, expected: %q", #data,
		#expectedTypes)
	end
...
function detectGlobalWrites()
	setmetatable(_G, {
		__newindex = function(t, key, val)
			rawset(_G, key, val)
			log('W', 'globals',
			debug.traceback('set new global variable: "' .. tostring(key) .. '"  to "' .. tostring(val) .. '"', 2, 1,
			false))
		end,
	})
end
