...
--- Returns the values of a `vec3`
--- @return number X, number Y, number Z
function LuaVec3:xyz()
	return self.x, self.y, self.z
end

--- Returns a `vec3` representation of the given string
--- @return LuaVec3
...
--- @deprecated
--- @type function
--- Usage of this function is not recommended, use `vec3:__tostring()` instead
vec3toString = LuaVec3.__tostring

--- Returns a table representation of a `vec3`
...
function LuaVec3:xnormOnLine(a, b)
	...
	return (bax * (self.x - a.x) + bay * (self.y - a.y) + baz * (self.z - a.z)) /
		(bax * bax + bay * bay + baz * baz + 1e-30) -- (b-a):dot(self-a) / (b-a):squaredLength()
end
...
function LuaVec3:setLerp(from, to, t)
	self.x, self.y, self.z = from.x + (to.x - from.x) * t, from.y + (to.y - from.y) * t,
		from.z +
		(to.z - from.z) *
		t -- monotonic
end
...
function LuaVec3:getBlueNoise3d()
	self.x, self.y, self.z = fractPos(self.x + 0.81917251339616443), fractPos(self.y + 0.6710436067037892084),
		fractPos(self.z + 0.54970047790197026)
	return self
end
...
function LuaVec3:getBluePointInSphere(radius)
	...
	for i = 1, 8 do -- seen up to 6
		local x, y, z = fractPos(bx + 0.81917251339616443 * i), fractPos(by + 0.6710436067037892084 * i),
			fractPos(bz + 0.54970047790197026 * i)
		local xc, yc, zc = x - 0.5, y - 0.5, z - 0.5
		if xc * xc + yc * yc + zc * zc <= 0.25 then
			self.x, self.y, self.z = x, y, z
			local r2 = radius * 2
...
function StackVec3.__add(a, b)
	local bx, by, bz = b:xyz()
	stackv3[stacki - 3], stackv3[stacki - 2], stackv3[stacki - 1] = stackv3[stacki - 3] + bx, stackv3[stacki - 2] + by,
		stackv3[stacki - 1] + bz
	return stackv3
end
...
function StackVec3.__unm(a)
	stackv3[stacki - 3], stackv3[stacki - 2], stackv3[stacki - 1] = -stackv3[stacki - 3], -stackv3[stacki - 2],
		-stackv3[stacki - 1]
	return stackv3
end
...
function StackVec3.__mul(a, b)
	...
	stackv3[stacki - 3], stackv3[stacki - 2], stackv3[stacki - 1] = stackv3[stacki - 3] * b, stackv3[stacki - 2] * b,
		stackv3[stacki - 1] * b
	return stackv3
end
...
function StackVec3:cross(b)
	...
	stackv3[stacki - 3], stackv3[stacki - 2], stackv3[stacki - 1] = ay * bz - az * by, az * bx - ax * bz,
		ax * by - ay * bx
	return stackv3
end
...
function StackVec3:normalized()
	...
	stackv3[stacki - 3], stackv3[stacki - 2], stackv3[stacki - 1] = stackv3[stacki - 3] * r, stackv3[stacki - 2] * r,
		stackv3[stacki - 1] * r
	return stackv3
end
...
function cardinalSpline(p0, p1, p2, p3, t, s, d1, d2, d3)
	...
	return t * t_1sq * sd2 * m1 + tt * t_1 * sd2 * m2 + t_1sq * (2 * t + 1) * p1 - tt * (2 * t - 3) * p2 +
		s * t_1 * (t * t_1 + tt) * (p2 - p1)
end
...
function catmullRomCentripetal(p0, p1, p2, p3, t, s)
	return cardinalSpline(p0, p1, p2, p3, t, s or 0.5, sqrt(p0:distance(p1)), sqrt(p1:distance(p2)),
		sqrt(p2:distance(p3)))
end
...
function monotonicSteffen(y0, y1, y2, y3, x0, x1, x2, x3, x)
	...
	local m1 = (sign(delta0) + sign(delta1)) *
		min(abs(delta0), abs(delta1), 0.5 * abs((x2x1 * delta0 + x1x0 * delta1) / (x2 - x0 + 1e-30)))
	local m2 = (sign(delta1) + sign(delta2)) *
		min(abs(delta1), abs(delta2), 0.5 * abs((x3x2 * delta1 + x2x1 * delta2) / (x3 - x1 + 1e-30)))
	local xx1 = x - x1
...
function overlapsOBB_OBB(c1, x1, y1, z1, c2, x2, y2, z2)
	...
	return abs(cc:dot(x1)) - d11 - d12 - d13 <= x1:squaredLength() and abs(cc:dot(y1)) - d21 - d22 - d23 <=
		y1:squaredLength()
		and abs(cc:dot(z1)) - d31 - d32 - d33 <= z1:squaredLength() and abs(cc:dot(x2)) - d11 - d21 - d31 <=
		x2:squaredLength()
		and abs(cc:dot(y2)) - d12 - d22 - d32 <= y2:squaredLength() and abs(cc:dot(z2)) - d13 - d23 - d33 <=
		z2:squaredLength()
end
...
--- Determines if an oriented bounding box (OBB) contains a sphere.
--- @param c1 LuaVec3 The center position of the OBB.
--- @param x1 LuaVec3 The first axis of the OBB.
--- @param y1 LuaVec3 The second axis of the OBB.
--- @param z1 LuaVec3 The third axis of the OBB.
--- @param c2 LuaVec3 The center position of the sphere.
--- @param r2 number The radius of the sphere.
--- @return boolean contained True if the sphere is contained within the OBB, false otherwise.
function containsOBB_Sphere(c1, x1, y1, z1, c2, r2)
	local cc = c1 - c2
	local x1len, y1len, z1len = x1:length(), y1:length(), z1:length()
	return abs(cc:dot(x1)) <= x1len * (x1len - r2) and abs(cc:dot(y1)) <= y1len * (y1len - r2) and
		abs(cc:dot(z1)) <= z1len * (z1len - r2)
end

--- Determines if an OBB contains a given sphere.
--- @param c1 LuaVec3 the center of the sphere.
--- @param r1 number the radius of the sphere.
--- @param c2 LuaVec3 the center of the OBB.
--- @param x2 LuaVec3 the normalized x-axis of the OBB.
--- @param y2 LuaVec3 the normalized y-axis of the OBB.
--- @param z2 LuaVec3 the normalized z-axis of the OBB.
--- @return boolean contained True if the OBB contains the sphere, false otherwise.
function containsSphere_OBB(c1, r1, c2, x2, y2, z2)
	local cc = c1 - c2
	local ccx1, cc_x2, y2z2, y2_z2 = cc + x2, cc - x2, y2 + z2, y2 - z2
	return max((ccx1 + y2z2):squaredLength(), (ccx1 + y2_z2):squaredLength(), (ccx1 - y2_z2):squaredLength(),
			(ccx1 - y2z2):squaredLength(),
			(cc_x2 + y2z2):squaredLength(), (cc_x2 + y2_z2):squaredLength(), (cc_x2 - y2_z2):squaredLength(),
			(cc_x2 - y2z2):squaredLength()) <= r1 * r1
end

--- Checks if a point is inside an oriented bounding box (OBB).
--- @param c1 LuaVec3 the center of the OBB.
--- @param x1 LuaVec3 the unit vector representing the X axis of the OBB.
--- @param y1 LuaVec3 the unit vector representing the Y axis of the OBB.
--- @param z1 LuaVec3 the unit vector representing the Z axis of the OBB.
--- @param p LuaVec3 the point to be checked.
--- @return boolean inside Whether the point is inside the OBB or not.
function containsOBB_point(c1, x1, y1, z1, p)
	local cc = c1 - p
	return abs(cc:dot(x1)) <= x1:squaredLength() and abs(cc:dot(y1)) <= y1:squaredLength() and
		abs(cc:dot(z1)) <= z1:squaredLength()
end

--- Returns whether a given point is inside an ellipsoid.
--- @param c1 LuaVec3 the center of the ellipsoid.
--- @param x1 LuaVec3 the x-axis direction vector of the ellipsoid.
--- @param y1 LuaVec3 the y-axis direction vector of the ellipsoid.
--- @param z1 LuaVec3 the z-axis direction vector of the ellipsoid.
--- @param p LuaVec3 the point to check.
--- @return boolean inside True if the point is inside the ellipsoid, false otherwise.
...
--- Checks if a point is contained within a cylinder.
--- @param cposa LuaVec3 the starting position of the cylinder.
--- @param cposb LuaVec3 the end position of the cylinder.
--- @param cR number the radius of the cylinder.
--- @param p LuaVec3 the point to check for containment.
--- @return boolean within Returns true if the point is within the cylinder, false otherwise.
...
--- Returns the signed distance from the center of an oriented bounding box (OBB) to a plane.
--- @param c1 LuaVec3 the center point of the OBB.
--- @param x1 LuaVec3 the x-axis vector of the OBB.
--- @param y1 LuaVec3 the y-axis vector of the OBB.
--- @param z1 LuaVec3 the z-axis vector of the OBB.
--- @param plpos LuaVec3 the position of a point on the plane.
--- @param pln LuaVec3 the normal vector of the plane.
--- @return number distance The signed distance between the center of the OBB and the plane.
function altitudeOBB_Plane(c1, x1, y1, z1, plpos, pln)
	return (c1 - plpos):dot(pln) + abs(x1:dot(pln)) + abs(y1:dot(pln)) + abs(z1:dot(pln))
end

--- Returns the signed distance of a plane along a ray.
--- @param rpos LuaVec3 The starting position of the ray.
--- @param rdir LuaVec3 The direction of the ray (must be normalized).
--- @param plpos LuaVec3 The position of a point on the plane.
--- @param pln LuaVec3 The normal of the plane (must be normalized).
--- @return number distance The signed distance of the plane on the ray.
function intersectsRay_Plane(rpos, rdir, plpos, pln)
	return min((plpos - rpos):dot(pln) / rdir:dot(pln), math.huge)
end

--- Calculates the intersection point between a ray and an oriented bounding box (OBB) in 3D space.
--- @param rpos LuaVec3 The starting position of the ray.
--- @param rdir LuaVec3 The direction of the ray (must be normalized).
--- @param c1 LuaVec3 The center position of the OBB.
--- @param x1 LuaVec3 The normalized X-axis of the OBB.
--- @param y1 LuaVec3 The normalized Y-axis of the OBB.
--- @param z1 LuaVec3 The normalized Z-axis of the OBB.
--- @return number distance1, number distance2 If the ray intersects the OBB, returns the two distances along the ray to the intersection points. Otherwise, returns infinity for both distances.
function intersectsRay_OBB(rpos, rdir, c1, x1, y1, z1)
	...
	local minhit, maxhit = max(min(dx1, dx2), min(dy1, dy2), min(dz1, dz2)),
		min(max(dx1, dx2), max(dy1, dy2), max(dz1, dz2))
	return (minhit <= maxhit and minhit or math.huge), maxhit
end

--- Calculates the intersection point between a ray and a sphere in 3D space.
--- @param rpos LuaVec3 the starting position of the ray.
--- @param rdir LuaVec3 the direction of the ray (must be normalized).
--- @param cpos LuaVec3 the center position of the sphere.
--- @param cr number the radius of the sphere.
--- @return number r1 The distance along the ray to the first intersection point, if the ray intersects the sphere.
--- @return number r2 The distance along the ray to the second intersection point, if the ray intersects the sphere.
--- If the ray does not intersect the sphere, returns infinity for both r1 and r2.
...
--- Calculates the intersection point between a ray and an ellipsoid in 3D space.
--- @param rpos LuaVec3 the starting position of the ray.
--- @param rdir LuaVec3 the direction of the ray (must be normalized).
--- @param c1 LuaVec3 the center position of the ellipsoid.
--- @param x1 LuaVec3 the first axis of the ellipsoid.
--- @param y1 LuaVec3 the second axis of the ellipsoid.
--- @param z1 LuaVec3 the third axis of the ellipsoid.
--- @return number r1 The distance along the ray to the first intersection point, if the ray intersects the ellipsoid.
--- @return number r2 The distance along the ray to the second intersection point, if the ray intersects the ellipsoid.
function intersectsRay_Ellipsoid(rpos, rdir, c1, x1, y1, z1)
	local invx1, invy1, invz1 = 1 / (x1:squaredLength() + 1e-30), 1 / (y1:squaredLength() + 1e-30),
		1 / (z1:squaredLength() + 1e-30)
	local cc = rpos - c1
...
--- Calculates the intersection point between a ray and a cylinder in 3D space.
--- @param rpos LuaVec3 the starting position of the ray.
--- @param rdir LuaVec3 the direction of the ray (must be normalized).
--- @param cposa LuaVec3 the position of one endcap of the cylinder.
--- @param cposb LuaVec3 the position of the other endcap of the cylinder.
--- @param cR number the radius of the cylinder.
--- @return number minhit The distance along the ray to the first intersection point, if the ray intersects the cylinder.
--- @return number maxhit The distance along the ray to the second intersection point, if the ray intersects the cylinder.
--- If the ray does not intersect the cylinder, returns infinity for both minhit and maxhit.
...
--- Calculates the intersection point between a ray and a triangle in 3D space.
--- @param rpos LuaVec3 the starting position of the ray.
--- @param rdir LuaVec3 the direction of the ray (must be normalized).
--- @param a LuaVec3 the first vertex of the triangle.
--- @param b LuaVec3 the second vertex of the triangle.
--- @param c LuaVec3 the third vertex of the triangle.
--- @return number distance The distance along the ray to the intersection point, if the ray intersects the triangle.
--- @return number barycentric_x The barycentric x-coordinate of the intersection point, if the ray intersects the triangle.
--- @return number barycentric_y The barycentric y-coordinate of the intersection point, if the ray intersects the triangle.
