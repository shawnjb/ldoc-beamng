...
scenetree.__newindex = function(...)
end -- disallow any assignments
		...
		if k ~= 'dataBlock' and k ~= 'parentGroup' then -- why do we need to exclude these?
			...
			if val == nil then
				...
				print(' * ' ..
					tostring(k) ..
					' = ' ..
					tostring(player[k]) .. ' [Types| Lua: ' .. type(val) .. ', C: ' .. (f.type or 'unknown') .. ']')
			end
		else
...
local function colorTableToRoundedColorString(color, metallicData)
	...
	return tostring(x) ..
		...
		" " ..
		tostring(w1) -- the TS sequence was like this
end
...
--- Returns a list of all currently spawned BeamNGVehicle objects in the level.
--- @return table allVehiclesCache A table containing all currently spawned BeamNGVehicle objects in the level.
...
function getTimeOfDay(asString)
	...
	if asString then
		return string.format("%02.f", hours) ..
			":" .. string.format("%02.f", mins) .. ":" .. string.format("%02.f", secs)
	else
...
function queueCallbackInVehicle(veh, geluaFunctionName, vluaCommand, ...)
	...
	local geluaCommand = string.format('local args = %s; %s(unpack(args, 1, table.maxn(args)))', 'deserialize(%q)',
		geluaFunctionName)
	local cmd = string.format('obj:queueGameEngineLua(string.format(%q, serialize({%s, unpack(%s)})))', geluaCommand,
		vluaCommand, serialize({ ... }))
...
function getMaxVehicleAmount(cap)
	return Engine.Platform.getCPUInfo() and clamp(Engine.Platform.getCPUInfo().coresPhysical or 1, 1, cap or math.huge) or
		4
end
...
function convertVehicleColorsToPaints(colorTable)
	if type(colorTable) ~= 'table' then
		log('W', 'vehiclePaint',
			'colorTable parameter should be a table of colors. type = ' .. type(colors) .. ' colors = ' .. dumps(colors))
		colorTable = {}
	end
	...
	for i = 1, colorTableSize do
		local paint = createVehiclePaint(
			{ x = colorTable[i][1], y = colorTable[i][2], z = colorTable[i][3], w = colorTable[i][4] }, {})
...
function validateVehicleDataColor(color)
	...
	if type(color) == 'string' then
		...
		validatedColor = string.format("%0.2f %0.2f %0.2f %0.2f", tonumber(components[1]), tonumber(components[2]),
			tonumber(components[3]), tonumber(components[4]))
	elseif type(color) == 'table' then
		validatedColor = string.format("%0.2f %0.2f %0.2f %0.2f", color[1] or 1, color[2] or 1, color[3] or 1,
			color[4] or 1)
	end
...
function sanitizeVehicleSpawnOptions(model, opt)
	...
	if not options.rot then
		if commands.isFreeCamera() or not playerVehicle then
			...
			options.rot = quatFromDir(vec3(playerVehicle:getDirectionVector()):normalized(),
				vec3(playerVehicle:getDirectionVectorUp()):normalized())
		end
	end
...
function extractVehicleData(vid)
	...
	if not vehicleData then
		...
		vehicleData.color = string.format("%0.2f %0.2f %0.2f %0.2f", vehicle.color.x, vehicle.color.y, vehicle.color.z,
			vehicle.color.w)
		...
		for vid, v in pairs(data) do
			...
			if vehicle then
				...
				if not name then
					...
					log("W", "",
						"Vehicle does not have name, using id as string instead: " .. dumps(vid) .. " -> " .. dumps(name))
				end
				result[name] = v
			else
...
function isPlayerVehConfig(vpath)
	...
	return string.startswith(FS:getFileRealPath(vpath), FS:getUserPath() .. "vehicles" .. osSep) and
		string.endswith(vpath, ".pc")
end
...
local comparisonOps = {
	...
	},
	{
		opSymbol = '>=',
		op = gte,
		opName = 'Greater Than or Equal To',
		opNameLc =
	...
	for path in string.gmatch(package.path, "([^;]+)") do
		...
		if file then
			...
			if string.find(filename, '/extensions/') then
				...
				content = "local logTag = '" ..
					modulenameVirt ..
					"'; local log = function(level, origin, msg) log(level, msg and ('" ..
					modulenameVirt ..
					".'..origin) or origin, msg) end;" ..
					content --if using all 3 logging args, then prefix the module name into 'origin'
				--print(content)
			end
