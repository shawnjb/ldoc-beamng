...
local function buildClusterBoundingBox(veh)
  ...
  for i, nodePosition in ipairs(nodePositions) do
    nodePositions[i] = debugDrawer:drawSphere(veh:getPosition() + nodePosition, 0.1, ColorF(1,0,0,1))
  end
  ...
  for i = 0,7 do
    debugDrawer:drawSphere(boundingBox:getPoint(i), 0.1, ColorF(0,1,0,1))
  end

  return boundingBox
end
--M.buildClusterBoundingBox = buildClusterBoundingBox
...
local function buildBoundingBox(veh)
  if not boxValid(veh.initialNodePosBB) then return end
  ...
  -- If the spawn position is not right side up, rotate it to horizontal
  if math.acos(up:cosAngle(localVehUp)) * 180/math.pi > 60 then
    up = localVehUp
    forward.z = 0
    forward = forward:normalized()
    rot = quatFromDir(forward, up)
  ...
  boundingBox:set2(mat, veh.initialNodePosBB:getExtents())
...
local function intersectingOtherVehicle(otherVeh, axis0, axis1, axis2, halfExtentsX, halfExtentsY, halfExtentsZ, bbCenter)
  ...
  local otherVehAxis1, otherVehAxis2, otherVehAxis3 = vec3(1,0,0), vec3(0,1,0), vec3(0,0,1)
  local otherBBHalfExtents = otherBB:getExtents() / 2
  local otherBBCenter = otherBB:getCenter()
  local otherNodeCount = otherVeh:getNodeCount()

  if otherNodeCount == 0 then -- if the other vehicle was also spawned in this frame, we need to build the BB ourselves
  ...
  if overlapsOBB_OBB(bbCenter, axis0 * halfExtentsX, axis1 * halfExtentsY, axis2 * halfExtentsZ, otherBBCenter, otherVehAxis1 * otherBBHalfExtents.x, otherVehAxis2 * otherBBHalfExtents.y, otherVehAxis3 * otherBBHalfExtents.z) then
    ...
    if otherNodeCount == 0 then -- if the otherVeh doesnt have nodes, we only go by the BB
      return true
    end
...
local function moveBBToGround(vehBB, raycastDown)
  ...
  for x = -1, 1 do
    for y = -1, 1 do
      if (y ~= 0 and x ~= 0) and not (y == 1 and x == 1) then
        ...
        --debugDrawer:drawSphere(startPos + downVec * detectedDist, 0.1, a and ColorF(1,0,0,1) or ColorF(0,1,0,1))
      end
    end
  end
  ...
  local center = rayCastHits[1].pos + right * halfExtentsX + forward * halfExtentsY + up * halfExtentsZ
  local mat = MatrixF()
  mat:setColumn(0, right)
  mat:setColumn(1, forward)
  mat:setColumn(2, up)
...
local function getBBOnGround(vehBB, vehOrigin)
  debugDrawer:drawSphere(vehOrigin, 0.1, ColorF(1,1,1,1))
  local newBB = OrientedBox3F()
  local mat = vehBB:getMatrix()
  mat:setPosition(vehBB:getCenter())
  newBB:set2(mat, vehBB:getHalfExtents() * 2)
  for i = 0,7 do
    --debugDrawer:drawSphere(hit.pos, 0.5, ColorF(1,1,1,1))
    debugDrawer:drawSphere(vehBB:getPoint(i), 0.1, ColorF(1,0,0,1))
  end

  moveBBToGround(newBB, true)
  -- TODO put some limit on the angle of the BB
  -- second raycast in direction of vehicle down
  moveBBToGround(newBB)
  for i = 0,7 do
    --debugDrawer:drawSphere(hit.pos, 0.5, ColorF(1,1,1,1))
    debugDrawer:drawSphere(newBB:getPoint(i), 0.1, ColorF(0,1,0,1))
  end

  local originalRot = quat(vehBB:getMatrix():toQuatF())
  ...
  local localRefNode = vehOrigin - vehBB:getCenter()
  localRefNode = diffRot * localRefNode
  localRefNode = newBB:getCenter() + localRefNode
  debugDrawer:drawSphere(localRefNode, 0.1, ColorF(1,1,1,1))

  --be:getPlayerVehicle(0):setPosRot(localRefNode.x, localRefNode.y, localRefNode.z, newRot.x, newRot.y, newRot.z, newRot.w)

  --debugDrawer:drawLine(center + up, 0.5, ColorF(1,0,0,1))
  --debugDrawer:drawSphere(center + forward, 0.5, ColorF(0,1,0,1))
  --debugDrawer:drawSphere(center + right, 0.5, ColorF(0,0,1,1))


  --table.sort(rayCastHits, function(a, b) return a.detectedDist < b.detectedDist end)
end

--M.getBBOnGround = getBBOnGround

local function isFloating(bb)
  local rayDist = getAutoplaceRange(bb)
  local axis2 = bb:getAxis(2)
  local bbCenter = bb:getCenter()

  if castRayStatic(bbCenter, -axis2, rayDist) >= rayDist then
    return true
  end
  return false
end
...
local factor = 2
local foundIntersectionOnFirstPos
local handledTraffic
local bestCandidate = {}
local function placeVehRec(vehBB, newVehID, vehDir, vehRight)
  recDepth = recDepth + 1
  if recDepth > 400 then
    ...
    if bestCandidate.bbCenter then
      log('I', logTag, "Using best found spawn position")
      local vehicle = be:getObjectByID(newVehID)
      vehicle:setPosition(vehicle:getPosition() + (bestCandidate.bbCenter - originalBB:getCenter()))
      vehicle:autoplace(false)
    else
      log('W', logTag, "Couldnt find suitable spawning position. Putting the vehicle a little higher.")
      local vehicle = be:getObjectByID(newVehID)
      vehicle:setPosition(vehicle:getPosition() + vec3(0,0,1))
    end
    return
  end

  -- Test different attributes of the candidate position
  visibilityPoint = visibilityPoint or originalBB:getCenter()
  ...
  if not wrongFloatingStatus and not foundIntersection and pointVisibilityStatus > 0 then
    if not bestCandidate.pointVisibilityStatus or pointVisibilityStatus > bestCandidate.pointVisibilityStatus then
      ...
      bestCandidate.bbCenter = vehBB:getCenter()
    end
  end
  ...
  if wrongFloatingStatus or foundIntersection or pointVisibilityStatus < 2 then
    local mat = vehBB:getMatrix()
    local offset = getNextOffset(vehDir, vehRight)
    mat:setPosition(originalBB:getCenter() + offset)
    newVehBB:set2(mat, vehBB:getHalfExtents() * factor)
    return placeVehRec(newVehBB, newVehID, vehDir, vehRight)
  end
  local vehicle = be:getObjectByID(newVehID)

  -- TODO dont move the vehicle object here
  vehicle:setPosition(vehicle:getPosition() + (vehBB:getCenter() - originalBB:getCenter()))
  vehicle:autoplace(false)
  return vehBB
end

local function placeVehicle(vehBB, newVehID, vehDir, vehRight)
  -- Reset variables
  gridX = -1
  gridY = -1
  vehicleNodes = {}
  ...
  if isIntersecting(vehBB, newVehID) or (visibilityPoint and getVisibilityStatus(visibilityPoint, vehBB) < 2) then
    foundIntersectionOnFirstPos = true
...
local function setSafePositionRec(veh)
  foundIntersectionOnFirstPos = false
  bestCandidate = {}

  local boundingBox, forward, right = buildBoundingBox(veh)
  if not boundingBox then return end
  local newBB = placeVehicle(boundingBox, veh:getId(), forward, right)
  if foundIntersectionOnFirstPos and not recursionLimitReached then
    -- Check position again after the vehicle got autoplaced
    setSafePositionRec(veh) -- TODO this shouldnt be necessary anymore because we do an "autoplace" for each tested position
  end
  if newBB and not handledTraffic and not checkOnlyStatics and removeTraffic then
...
local function setSafePosition(veh)
  recursionLimitReached = false
  handledTraffic = false
  recDepth = 0
  setSafePositionRec(veh)
  removeTraffic = true
end
...
local function setVehicleObject(veh, options)
  ...
  veh:setMetallicPaintData({paint, paint2, paint3})
...
local function safeTeleport(veh, pos, rot, checkOnlyStatics_, visibilityPoint_, removeTraffic_, centeredPosition)
  --local initialBB = buildClusterBoundingBox(veh)
  rot = rot or quat(0,0,0,-1)
  rot = quat(0,0,1,0) * rot
  veh:setPosRot(pos.x, pos.y, pos.z, rot.x, rot.y, rot.z, rot.w)
  if centeredPosition then
    centerVehicle(veh, pos, rot)
  end
  airSpawn = not veh:autoplace(false)
...
local function teleportToLastRoadCallback(data)
  ...
  safeTeleport(veh, pos, rot)
end

local function teleportToLastRoad(veh)
  veh = veh or be:getPlayerVehicle(0)
  if not veh then return end
  queueCallbackInVehicle(veh, "spawn.teleportToLastRoadCallback", "recovery.recoveryPoints")
end
...
local function spawnCamera()
  ...
  for _,camName in ipairs({"gameCamera", "freeCamera"}) do
    ...
    local cam = scenetree.findObject(camName)
    if not cam then
