...
local M = {}
M.dependencies = { "gameplay_missions_missionManager", "gameplay_missions_startTrigger", "freeroam_bigMapMode",
...
local function orderStarKeysDefaultThenBonus(mission)
	...
	return arrayConcat(deepcopy(defaultKeysSorted), deepcopy(bonusKeysSorted)), defaultKeysSorted, bonusKeysSorted,
	defaultStarKeysCache, bonusStarKeysCache
end
...
local function sanitizeMissionAfterCreation(mission)
	...
	local sortedStars, defaultKeysSorted, bonusKeysSorted, defaultStarKeysCache, bonusStarKeysCache =
	orderStarKeysDefaultThenBonus(mission)
	...
	mission.getRandomizedAttempt = mission.getRandomizedAttempt or
	function() return gameplay_missions_progress.testHelper.randomAttemptType(), {} end
end
...
local function recursiveRemoveNestedFromCondition(mId, cond)
	if cond.nested then
		if cond.type == 'multiAnd' or cond.type == 'multiOr' then
			...
			"In Mission " ..
			mId ..
			": Nested condition found in a condition that should not have nested in it. Re-save the mission to apply fix.")
		end
	end
end
...
local function sanitizeMission(missionData, filepath)
	...
	if missionData.name == "" or string.find(missionData.name, ".json") then
		log("E", "",
		"Incorrect 'name' field, please clean it up (no filepaths, no underscores, etc): " ..
		dumps(missionData.name) .. " at: " .. filepath)
...
local function getFilesData()
	if not filesData then
		...
		log("D", "",
		"Loaded " ..
		#filesData .. " total missions: " .. fromFilesCount .. " from files, " .. genCount .. " from generators.")
	end
	return filesData
end
	...
	if not missions then
		...
		for _, missionData in ipairs(getFilesData()) do
			...
			if not missionConstructor then
				log("E", "",
				"Mission " .. dumps(infoPath) .. " did not specify a valid missionType: " ..
				dumps(missionData.missionType))
				goto continue
			end

			-- actually construct the mission
			local result, mission, add = xpcall(function() return missionConstructor(deepcopy(missionData)) end,
			debug.traceback)
			if add == true or not mission then
				log("E", "",
				"Unable to construct mission " ..
				dumps(missionData.id) ..
				" of type " .. dumps(missionData.missionType) .. ", its constructor returned " .. dumps(add))
				goto continue
			end
			if type(mission) == 'string' then
				log("E", "",
				"Unable to construct mission " ..
				dumps(missionData.id) .. " of type " .. dumps(missionData.missionType) .. ", something went wrong:")
				dump(mission)
				goto continue
			end
