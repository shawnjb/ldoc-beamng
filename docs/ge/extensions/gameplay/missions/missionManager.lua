...
local function startWithFade(mission, userSettings)
  if not mission then
    log("E", logTag, "Couldn't start mission, mission id not found.")
    return true
  end
  ...
  if taskData.active then
    log("W", logTag, "Attempting to start mission while there is an active task.")
    return
  end
    ...
    if delayedStartFromWithinMission then
      if not delayedStartFromWithinMission.currMission then
        ...
        if fail then
          ...
          log("I", logTag, "Delayed mission now loading.")
          fadedScreen = true
        end
