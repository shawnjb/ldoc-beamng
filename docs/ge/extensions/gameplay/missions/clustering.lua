...
local function extractZoneData(zoneNames)
  ...
  if not garageSites then
    log("E","","Garagesites missing!")
  end
  if not zoneNames or not next(zoneNames) then
    log("E","","zoneNames empty or missing!: " .. dumps(zoneNames))
  end
  ...
  if garageSites and zoneNames then
    ...
    for _, zoneName in ipairs(zoneNames or {}) do
      ...
      if zone and not zone.missing then
        ...
        log("W","","Missing Zone?: " .. dumps(zoneName) .. " of " .. dumps(tableKeys(garageSites.zones.byName)))
      end
    end
    ...
    if career_career.isCareerActive() then
      for i, garagePoint in ipairs(levelInfo.garagePoints or {}) do
        ...
        if success then
          ...
          log("E","","Could not load garage zone data! " .. dumps(garagePoint.id))
        end
      end
    end
    if career_career.isCareerActive() or settings.getValue("enableGasStationsInFreeroam") then
      for i, gasStationPoint in ipairs(levelInfo.gasStationPoints or {}) do
        ...
        if success then
          ...
          log("E","","Could not load gas station zone data! " .. dumps(gasStationPoint))
        end
      end
    end
...
local function merge(allClusters, cluster, settings)
  ...
  if #cluster == 1 then
    ...
    local typeCount = {}
    for type, _ in pairs(iconPriority) do
      typeCount[type] = 0
    end
