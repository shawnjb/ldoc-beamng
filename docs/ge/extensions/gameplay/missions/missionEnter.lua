    ...
    if cluster then
      ...
      if marker then
        ...
        if visibleIds[id] and not photoModeOpen and not editor.active then
          ...
          if ((career_career and career_career.isCareerActive()) or marker.forceVisible) and
             (not freeroam_bigMapMode.markerToNavigateTo == not marker.forceVisible) then
              showMarker = true
          else       -- check if marker contains a mission
            if cluster.hasType['mission'] and settings.getValue("showMissionMarkers") then
              showMarker = true
            end
            if cluster.hasType['gasStationPoint'] and settings.getValue("enableGasStationsInFreeroam") then
              showMarker = true
            end
          end
        end
