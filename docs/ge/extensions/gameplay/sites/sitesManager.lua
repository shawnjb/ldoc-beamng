...
M.loadAllLevelSites = function()
  ...
  log("I", "loadAllLevelSites", "Loaded " .. loadCount .." sites files.")
end
...
M.onSerialize = function()
  if sitesByLevel == nil or sitesByFilepath == nil or sitesByLevelByName == nil then
    return nil
  end
  local ret = {
    sitesByLevel = {},
...
M.getSitesByLevel = function()
  if not sitesByLevel then M.loadAllLevelSites() end
  return sitesByLevel
end
M.getCurrentLevelSites = function()
  if not sitesByLevel then M.loadAllLevelSites() end
  return sitesByLevel[getCurrentLevelIdentifier()] or {}
end
M.getCurrentLevelSitesByName = function(name)
  if not sitesByLevelByName then M.loadAllLevelSites() end
  return (sitesByLevelByName[getCurrentLevelIdentifier()] or {})[name]
end
M.getSitesByFilepath = function()
  if not sitesByFilepath then M.loadAllLevelSites() end
  return sitesByFilepath
end
return M
