...
local function checkPlayer(id, prevId) -- checks if the player data needs to be inserted
  if trafficAiVehsList[1] then
    local obj = be:getObjectByID(id)

    if obj and obj:isPlayerControlled() then
      if traffic[id] and traffic[id].alpha ~= 1 then -- if vehicle was invisible, show it
        obj:setMeshAlpha(1, '')
        traffic[id].alpha = 1
      else
        ...
        prevId = prevId or 0
        local prevObj = be:getObjectByID(prevId)
        local isWalking = (obj and obj.jbeam == 'unicycle') or (prevObj and prevObj.jbeam == 'unicycle')
        if isWalking and traffic[id] and traffic[prevId] and traffic[prevId].pursuit.mode > 0 then
          -- walk mode changed during an active pursuit
          local mode = traffic[prevId].pursuit.mode
          gameplay_police.setPursuitMode(0, prevId)
          traffic[id].queuedFuncs.pursuitChange = {timer = 0.1, func = gameplay_police.setPursuitMode, args = {mode, id}}
        end
      end
    end
  end
...
local function doTraffic(dt, dtSim) -- various logic for traffic; also handles when to respawn traffic
  ...
  local vehCount = 0
  for i, id in ipairs(tableKeysSorted(traffic)) do -- ensures consistent order of vehicles; TODO: optimize this
    vehCount = vehCount + 1
    local veh = traffic[id]
    if veh then
      local obj = be:getObjectByID(id)
