...
function C:init(id, role)
  ...
  if obj.jbeam == 'unicycle' then
    modelType = 'car' -- temporary?
  end
...
function C:onRefresh() -- triggers whenever vehicle data needs to be refreshed
  if self.isAi then
    ...
    if self.vars.aiMode == 'traffic' then
      obj:queueLuaCommand('ai.setParameters({turnForceCoef = 0.1, awarenessForceCoef = 0.1})') -- temporary, for better driving
    end
  end
