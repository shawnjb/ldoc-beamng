...
function C:getMissionIssues(m)
  ...
  if not m.additionalAttributes.difficulty then
    table.insert(issues, {label = 'No difficulty set!', severity='warning'})
  end
  ...
  if self.mission.author == nil or self.mission.author == "" then
    table.insert(issues, {label = 'No Author set!', severity='warning'})
  end
  if self.mission.date == nil or self.mission.date == 0 then
    table.insert(issues, {label = 'No Date set!', severity='warning'})
  end
  return issues
end
