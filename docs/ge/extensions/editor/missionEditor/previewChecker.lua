...
function C:getMissionIssues(m)
  ...
  if not self.previewFound then
    ...
    if self.previewImage.size.x ~= 1920 or self.previewImage.size.y ~= 1080 then
      table.insert(issues, {label = string.format('Preview file dimensions off! %d x %d instead of 1920 x 1080!', self.previewImage.size.x, self.previewImage.size.y)})
    end
  end
  ...
  elseif not self.thumbFound and self.previewFound then
    ...
    local thumbSizeInKb = getFileSize(self.thumbFile)/1024
    if thumbSizeInKb > 100 then
      table.insert(issues, {label = string.format('Thumbnail file too big! Is %d but should be < 100kb', thumbSizeInKb),  severity='warning'})
    end
  end
  return issues
end
