...
-- Highlighting refers to clicking on the "eye" icons
...
local function selectReset(vehID)
  ...
  if vehObj then
    if vehsPartsHighlighted[vehID] then
      ...
      -- defaults here if user didn't highlight any parts yet using UI
      local alpha = vehObj:getSpawnMeshAlpha()
...
local function clearVehicleHighlights(vehID)
  if vehID then
    vehsPartsHighlighted[vehID] = nil
  end
end

-- selecting refers to hovering over a part in the UI
