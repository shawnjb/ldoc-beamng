...
C.name = 'Nonlinear Smoother'
C.description = 'Provides nonlinear smoothing for a given signal.'
C.category = 'simple'
...
function C:work()
  ...
  if not sample then
    ...
    self.pinOut.value.value = self.smoother:getWithRate(sample,dt,rate)
  end
end
