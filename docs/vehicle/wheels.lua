...
local brakeTorqueLimits = {left = 0, right = 0}
local brakeTorqueLimitCache = {left = 0, right = 0}
local absPulse = 0
...
local function nodeCollision(p)
  ...
  if node then
    ...
    if wheelID then
      M.wheelRotators[wheelID].lastTreadContactNode = collisionNodeId
...
local function updateBrakeABS(wd, brake, invAirspeed, airspeed, airspeedCutOff, dt)
  if brake > 0 and wd.brakeTorque > 0 then
    ...
    if wd.absTimer <= 0 then
      ...
      wd.absActive = (nonABSBrakingTorque - desiredBrakingTorque > 100) and ABSCoef < 0.9
      absActive = wd.absActive or absActive
      absActiveLastPulse = absActive

      brakeTorqueLimitCache[wd.oppositeWheelSide] = max(desiredBrakingTorque * 1.15, brakeTorqueLimitCache[wd.oppositeWheelSide])
      desiredBrakingTorque = min(desiredBrakingTorque, brakeTorqueLimits[wd.ownWheelSide])

      wd.lastSlipError = slipError

      return desiredBrakingTorque
    else
      brakeTorqueLimitCache[wd.oppositeWheelSide] = brakeTorqueLimits[wd.oppositeWheelSide]
      absActive = absActive or absActiveLastPulse
      return wd.desiredMainBrakingTorque
    end
  else
    ...
    brakeTorqueLimitCache[wd.oppositeWheelSide] = maxDoubleBrakeTorque
    return 0
  end
end
...
local function updateWheelVelocities(dt)
  ...
  brakeTorqueLimitCache.left = 0
  brakeTorqueLimitCache.right = 0
  ...
  for i = 0, initialWheelCountDec do
    ...
    if not wd.isBroken then
      ...
      if wd.useDefaultBrakeInput then
        wd.desiredMainBrakingTorque = wd:updateBrake(brake, invAirspeed, airspeed, airspeedCutOff, dt)
        wd.desiredBrakingTorque = max(wd.desiredMainBrakingTorque, wd.parkingTorque * parkingbrakeInput)
      end
    end
  end
  ...
  brakeTorqueLimits.left = brakeTorqueLimitCache.left
  brakeTorqueLimits.right = brakeTorqueLimitCache.right
end
...
local function resetWheels()
  ...
  brakeTorqueLimits.left = maxBrakeTorque * 2
  brakeTorqueLimits.right = maxBrakeTorque * 2
end
