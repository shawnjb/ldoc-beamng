...
local function updateSimpleControlButtons()
  local color = n2oData.isArmed and (n2oData.isActive and "3096F1" or "98FB00") or "FFFFFF"
  extensions.ui_simplePowertrainControl.setButton(M.name, "N2O", "powertrain_n2o", color, n2oData.tankRatio,
  string.format("controller.getController(%q).toggleActive()", M.name))
end
...
local function displayState()
  guihooks.message(
  "Nitrous Oxide Injection: " .. ((electrics.values[armElectricsName] or 0) >= 1 and "Armed" or "Disarmed"), 5,
  "vehicle.powertrain.nitrousOxideInjection")
end
