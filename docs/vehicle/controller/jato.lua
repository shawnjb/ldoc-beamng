...
local function updateGFX(dt)
  ...
  if jatoInput > 0 and lastJatoInput <= 0 then
    for i = 1, thrusterCount, 1 do
      thrusterLoops[i] = thrusterLoops[i] or obj:createSFXSource2(thrusterLoopName, "AudioDefaultLoop3D", "jatoThrusterLoop", thrusterNodes[i][1], 0)
      obj:setVolume(thrusterLoops[i], jatoInput)
      obj:cutSFX(thrusterLoops[i])
      obj:playSFX(thrusterLoops[i])
    end
  end

  if jatoInput <= 0 and lastJatoInput > 0 then
    for i = 1, thrusterCount, 1 do
      obj:stopSFX(thrusterLoops[i] or -1)
    end
  end
