    ...
    for _, wheelGroup in pairs(wheelGroupData) do
      ...
      if groupName and uiName and wheelNames and #wheelNames > 0 and maxPressure and minPressure and groupEqualizeFlowLimit then
        ...
        for _, wheelName in pairs(wheelNames) do
          ...
          if wheelId then
            local pressureGroup = wheels.wheels[wheelId].pressureGroup
            if pressureGroup then
              local pressureGroupId = v.data.pressureGroups[pressureGroup]
              ...
              log("E", "tirePressureControl.initSecondStage", string.format("Can't find pressure group for wheel %q", wheelName))
            end
          else
