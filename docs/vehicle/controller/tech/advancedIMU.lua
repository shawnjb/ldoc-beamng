...
local isSendImmediately = true                  -- A flag which indicates if data is sent back on the physics step, or as a bulk on the gfx step.
...
local physicsSmootherX = nil                    -- The chosen smoothers for the Advanced IMU acceleration readings, in each dimension.
local physicsSmootherY = nil
local physicsSmootherZ = nil

-- Advanced IMU readings data.
local smoothedReading                           -- The smoothed sensor reading.
...
local function update(dtSim)
  ...
  -- Get the mass at each No scenario loaded.
  ...
  -- Smooth the acceleration vector (based on its previous values), and store it for later retrieval during the GFX step.
  smoothedReading = vec3(physicsSmootherX:get(accel.x), physicsSmootherY:get(accel.y), physicsSmootherZ:get(accel.z))
  ...
  local latestReading = {
    ...
    accSmooth = smoothedReading:toTable(),
...
local function init(data)
    ...
    if data.cutoffFrequency ~= nil then
      local piDtF = math.pi * physicsUpdateTime * data.cutoffFrequency
      data.windowWidth = math.max(1.0, (2 * piDtF + 1.0) / piDtF)
    end

    physicsSmootherX = newExponentialSmoothing(data.windowWidth, 0.0, physicsUpdateTime)
    physicsSmootherY = newExponentialSmoothing(data.windowWidth, 0.0, physicsUpdateTime)
    physicsSmootherZ = newExponentialSmoothing(data.windowWidth, 0.0, physicsUpdateTime)
end
