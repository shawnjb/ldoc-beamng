...
-- LookUp Parameters
local lookupParameters = {
  LOOKUP_DEVICE_TYPE = "devicetype", -- Lookup channel by Device type (see PCAN devices e.g. PCAN_USB)
  LOOKUP_DEVICE_ID = "deviceid", -- Lookup channel by device id
  LOOKUP_CONTROLLER_NUMBER = "controllernumber", -- Lookup channel by CAN controller 0-based index
  LOOKUP_IP_ADDRESS = "ipaddress" -- Lookup channel by IP address (LAN channels only)*/
}
...
local canBus = ffi.load("PCANBasic.dll")
ffi.cdef [[
//  PCANBasic.h
//
//  ~~~~~~~~~~~~
...
local function initPCAN()
  local status = canBus.CAN_Initialize(canBus.PCAN_USBBUS1, canBus.PCAN_BAUD_250K, 0, 0, 0)
  if status == canBus.PCAN_ERROR_OK then
    print("PCAN-USB (PCAN_USBBUS1) was initialized!")
  else
    print(string.format("ERROR! PCAN-USB (PCAN_USBBUS1) was NOT initialized! (Status code: 0x%x)", status))
  end
end
