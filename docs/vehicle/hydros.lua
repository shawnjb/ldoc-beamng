...
local ffbUnsafeFrequency
local function notifyUIffbUnsafe()
  obj:queueGameEngineLua(string.format("extensions.core_input_bindings.ffbUnsafeFrequencySet(deserialize(%q))", serialize(ffbUnsafeFrequency)))
end
    ...
    if FFBID >= 0 then
      if ffbConfig.ff_max_force and ffbConfig.ff_max_force ~= 0 then
        ...
        if ffbparams then
          ...
          ffbUnsafeFrequency = {}
          ffbUnsafeFrequency.isSafe = true
          if automaticRate then
            ...
            if finalFrequency > safeFrequency then
              ...
              ffbUnsafeFrequency.isSafe = false
            end
          end
          ffbUnsafeFrequency.safeFrequency = safeFrequency
          ffbUnsafeFrequency.finalFrequency = finalFrequency
          ...
          log("D", "hydros.init", "Force Feedback motor found for steering hydro. physicsID: "..dumps(obj:getId())..", FFBID: "..dumps(FFBID)..", ForceCoef "..M.wheelFFBForceCoef..", Smoothing "..wheelFFBSmoothing..", Update rate: "..msgDriver..", "..msgSafe..", "..msgSelected..", "..msgUsed.." ("..(automaticRate and "auto" or "manual")..")")
...
M.notifyUIffbUnsafe = notifyUIffbUnsafe
return M
