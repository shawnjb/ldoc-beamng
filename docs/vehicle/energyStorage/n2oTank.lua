...
local function setRemainingMass(storage, mass)
  storage.storedEnergy = min(max(mass, 0), storage.capacity) * storage.energyDensity
end
...
local function setPartCondition(storage, odometer, integrity, visual)
  ...
  if type(integrity) == "number" then
    ...
    integrityState = {
      remainingMass = storage.capacity * integrityValue
    }
  end

  storage:setRemainingMass(integrityState.remainingMass or storage.capacity)
end

local function getPartCondition(storage)
  local integrityState = {
    remainingMass = storage.remainingMass
  }
...
local function new(jbeamData)
  ...
  storage.initialStoredEnergy = storage.capacity * storage.energyDensity
  storage.remainingRatio = storage.initialStoredEnergy > 0 and storage.storedEnergy / storage.initialStoredEnergy or 0
