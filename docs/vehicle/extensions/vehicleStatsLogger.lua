...
local function log(level, msg)
  _log(level, "vehicleStatsLogger", msg)
end
...
local function addEngineModule()
  ...
  updateDeviceStates()
  if devices == nil then
    log("E", "no devices found in powertrain")
    return
  end

  if devices.mainEngine == nil then
    log("D", "no engine found")
    return
  end
...
local function applySettingsFromJSON(fpath)
  ...
  log("I", "importing settings from file: " .. fpath)
  ...
  --     log("D", moduleID..": "..tostring(useModule))
  -- end
end

local function writeSettingsToJSON(fpath)
  log("I", "exporting settings to JSON: " .. fpath)
  local s = {}
  ...
  if not jsonWriteFile(fpath, s, true, 0) then
    log("E", "failed writing settings to file")
  end
end
