...
local function initSecondStage()
  ...
  for wi1, wd1 in pairs(wheels.wheels) do
    ...
    if debug then print(string.format("----- '%s' = %s, %s", wd1.name, long1, side1)) end
      ...
      for wi2, wd2 in pairs(wheels.wheels) do
        ...
        if long2 == "R" then -- this is a rear wheel
          if side1 == nil then -- we don't know the side of front wheel (maybe it's a 3-wheel pigeon)
            ...
            if debug then print(string.format("    * '%s' = %s, %s", wd2.name, long2, side2)) end
...
local function dynamicInputRateKbd2(v, curx)
  ...
  -- centering by lifting key:
  if v == 0 then return understeerInRateMult * kbdInRateMult end

  -- centering by pressing opposite key:
  if signmov ~= signx then return understeerInRateMult * kbdInRateMult * 1.5 end

  -- recovering from oversteer:
  if signv == signgx or signmov == signgx or signx == signgx then return understeerInRateMult * kbdInRateMult * 1.8 end

  -- not enough data, fallback case
  local speed = electrics.values['wheelspeed']
  if speed == nil then return understeerInRateMult * kbdInRateMult end
...
local function inputStabilization(st, dt, filter)
  ...
  if not (useLimit or useSlowdown or useStabilization or useUndersteerReduction) then return st end
...
local function updateGFX(dt)
  ...
  for k, e in pairs(M.state) do
    ...
    if angle > 0 and k == "steering" then filter = FILTER_DIRECT end -- enforce direct filter if user has chosen an angle for steering binding


    if filter == FILTER_DIRECT then
      if k ~= "steering" then
        -- use direct matching for non-steering inputs (such as throttle, brake, etc)
        ival = e.val
      else
