...
local function updateGFX(dt)
      ...
      if volImpact > 0.004 then
        if (mat1 == 3 or mat2 == 3) then
          if impactPlasticEvent then
            --print(string.format("%d: Impact Plastic (%.2f) -> %q", objectId, volImpact, impactPlasticEvent))
            ...
            sounds.playSoundOnceFollowNode(impactPlasticEvent, breakNode, volImpact)
          end
        elseif (mat1 == 2 or mat2 == 2) then
          if impactMetalEvent then
            --print(string.format("%d: Impact Metal (%.2f) -> %q", objectId, volImpact, impactMetalEvent))
            ...
            sounds.playSoundOnceFollowNode(impactMetalEvent, breakNode, volImpact)
          end
        elseif impactGenericEvent then
          --print(string.format("%d: Impact Generic (%.2f) -> %q", objectId, volImpact, impactGenericEvent))
          ...
          -- sounds.playSoundOnceFollowNode("event:>Destruction>Generic>generic_impact", breakNode, volImpact)
          sounds.playSoundOnceFollowNode(impactGenericEvent, breakNode, volImpact)
        end
      end
    else
      if volBreak > 0.004 and breakGenericEvent then
  ...
  for bi, snd in ipairs(beamSounds) do
    ...
    -- DON'T FORGET THERE IS SMOOTHING ON THE VOLUME RTPC WHICH ALTERS FINAL RESULT
    -- streams.drawGraph('susp VOL w'..bi, {value = volume, min = 0, max = 1})
  ...
  if aeroSpeed > 3 and windSoundEvent then
    ...
    windSound = windSound or createSoundObj(windSoundEvent, "AudioDefaultLoop3D", "WindTestSound", getSoundscapeNode("horn") or M.refNode)
  ...
  for wi, wd in pairs(wheels.wheels) do
    ...
    if asphaltContactSmooth > maxContact then
      ...
      rigidSkidVolume = slip * wheelSound.tirePropertiesSlip * 0.02 * asphaltContactSmooth
    ...
    wheelSound.rigidRoll:setVolumePitch(rigidRollVolume, rigidRollPitch, rigidRollColor, rigidSurfaceType)
    wheelSound.rigidSkid:setVolumePitch(rigidSkidVolume, rigidSkidPitch, rigidSkidSlip + 0.5, rigidSurfaceType)
    ...
    if dirtContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        local kickupVolume = min(1, wheelPeripherySpeedKickup * 0.002 * wheelSound.tirePropertiesKickup)
        playSoundOnceAtNode("event:>Surfaces>kickup_dirt", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
    ...
    -- LOOSE dirtDusty
    local dirtdustyContactSmooth = wheelSound.dirtContactSmoother:getUncapped(boolToNum[mat == 14 and isRubberTire], dt)
    if dirtdustyContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        local kickupVolume = min(1, wheelPeripherySpeedKickup * 0.002 * wheelSound.tirePropertiesKickup)
        playSoundOnceAtNode("event:>Surfaces>kickup_dirtDusty", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
        wheelSound.looseSurfaceKickupLimit = randomGauss3() * 20 / wheelSound.tirePropertiesKickup
    ...
    if grassContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        ...
        playSoundOnceAtNode("event:>Surfaces>kickup_grass", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
    ...
    if gravelContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        ...
        playSoundOnceAtNode("event:>Surfaces>kickup_gravel", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
        wheelSound.looseSurfaceKickupLimit = randomGauss3() * 16 / wheelSound.tirePropertiesKickup
    ...
    if mudContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        local kickupVolume = min(1, wheelPeripherySpeedKickup * 0.0024 * wheelSound.tirePropertiesKickup)
        playSoundOnceAtNode("event:>Surfaces>kickup_mud", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
    ...
    if rockContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        local kickupVolume = min(1, wheelPeripherySpeedKickup * 0.0024 * wheelSound.tirePropertiesKickup)
        playSoundOnceAtNode("event:>Surfaces>kickup_rock", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
        wheelSound.looseSurfaceKickupLimit = randomGauss3() * 8 / wheelSound.tirePropertiesKickup
    ...
    if sandContactSmooth > maxContact then
      ...
      if wheelSound.looseSurfaceKickupLimit <= 0 and wheelPeripherySpeedKickup > 2 then
        ...
        playSoundOnceAtNode("event:>Surfaces>kickup_sand", wd.node1, kickupVolume, wheelSound.tirePropertiesVolRoll, wheelSound.tirePropertiesPitch, 1)
    ...
    wheelSound.looseRoll:setVolumePitch(looseRollVolume, looseRollPitch, looseRollDepth, looseSurfaceType)
    wheelSound.looseSkid:setVolumePitch(looseSkidVolume, looseSkidPitch, looseSkidDepth, looseSurfaceType)
    ...
    if rumbleStripContactSmooth > 0 then
      ...
      if peakForce > 0 and wd.obj:getPeakPeriod() * vehicleSpeed > 0.0 then
        ...
        playSoundOnceAtNode("event:>Surfaces>roll_rumblestrip", wd.node1, volume, pitch, wheelSound.tirePropertiesVolRoll)
      ...
      for _, bm in pairs(v.data.beams) do
        if bm.soundFile ~= nil then
          ...
          if soundTable.clip then
            ...
            -- print (string.format("Default Suspension Settings color=%.2f  attack=%.0f  volume=%.2f  decay=%.0f  pitch=%.2f  maxStress=%.0f", bm.colorFactor, attackFactor, bm.volumeFactor, decayFactor, bm.pitchFactor, bm.maxStress))
