...
local heatTickData = {
  engineDeltaTempSmoother = newTemporalSmoothing(1000, 0.0001),
  exhaustEnableTickSmoother = newTemporalSmoothing(1, 10),
  engineTickBucket = 0,
  exhaustTickBucket = 0,
  turboTickBucket = 0,
...
local function updateMechanicsGFX(dt)
  ...
  elseif damageTracker.getDamage("engine", "oilLevelCritical") then
    damageTracker.setDamage("engine", "oilLevelCritical", false, false)
  end
  ...
  if oilStarvingTimer >= oilStarvingTimerThreshold then
    if not damageTracker.getDamage("engine", "oilStarvation") then
      guihooks.message("vehicle.engine.starvedOfOil", 5, "vehicle.damage.oilstarvation")
  ...
  elseif damageTracker.getDamage("engine", "oilLevelTooHigh") then
    damageTracker.setDamage("engine", "oilLevelTooHigh", false, false)
  end
...
local function updateHeatTicksGFX(dt)
  ...
  local deltaTExhaust = heatTickData.lastExhaustTemperature - M.exhaustTemperature
  local deltaTExhaustRate = deltaTExhaust / dt
  --print((heatTickData.lastExhaustTemperature - M.exhaustTemperature) / dt)
  heatTickData.exhaustTickDelay = max((heatTickData.exhaustTickDelay or 3) - dt, 0)
  if deltaTExhaustRate <= 0 then
    heatTickData.exhaustTickDelay = 3
  end
  --print(deltaTExhaustRate)

  deltaTExhaust = min(deltaTExhaust / dt, 0.5) * dt
  heatTickData.exhaustTickBucket = max(heatTickData.exhaustTickBucket + deltaTExhaust * dt * 100, 0)

  local isExhaustHotEnough = M.exhaustTemperature >= 50
  --print(string.format("%.3f / %.3f", heatTickData.exhaustTickBucket, heatTickData.exhaustTickBucketThreshold))
  if heatTickData.exhaustTickBucket > heatTickData.exhaustTickBucketThreshold and isExhaustHotEnough and heatTickData.exhaustTickDelay <= 0 then
    local node = nodes.exhaust[random(#nodes.exhaust)]
    sounds.playSoundOnceFollowNode("event:>Engine>Cooldown>Test_Oneshot", node, 0.1, 1)
  ...
  local deltaTEngine = heatTickData.engineDeltaTempSmoother:getUncapped(heatTickData.lastEngineBlockTemperature - M.engineBlockTemperature, dt)
  deltaTEngine = min(deltaTEngine / dt, 0.2) * dt
  heatTickData.engineTickBucket = max(heatTickData.engineTickBucket + deltaTEngine * dt * 100, 0)
  --print(deltaTEngine / dt)

  local isEngineHotEnough = M.engineBlockTemperature >= 50
  --print(string.format("%.3f / %.3f", heatTickData.engineTickBucket, heatTickData.engineTickBucketThreshold))
  if heatTickData.engineTickBucket > heatTickData.engineTickBucketThreshold and isEngineHotEnough then
...
local function beamBroke(id)
  if exhaustBeams and exhaustBeams[id] then
    ...
    if not damageTracker.getDamage("engine", "exhaustBroken") then
      ...
      guihooks.message("vehicle.drivetrain.exhaustDamaged", 4, "vehicle.damage.exhaust")
    end
  end
end
