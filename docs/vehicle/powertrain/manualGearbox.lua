...
local function setGearIndex(device, index)
  ...
  if oldIndex ~= device.gearIndex and device.gearIndex ~= 0 and avDifference >= 0.5 and device.parent.clutchRatio then
    ...
    if damage > device.gearDamageThreshold * 0.01 and device.gearDamages[device.gearIndex] < device.gearDamageThreshold then
      guihooks.message({txt = string.format("Gear %g damaged (%d%%), please use the clutch!", device.gearIndex, device.gearDamages[device.gearIndex] / device.gearDamageThreshold * 100), context = {}}, 5, "vehicle.damage.gears")
    end
  end
  ...
  if device.gearDamages[device.gearIndex] >= device.gearDamageThreshold and device.gearRatios[device.gearIndex] ~= 0 then
    ...
    guihooks.message({txt = string.format("Gear %g permanently broken", device.gearIndex), context = {}}, 5, "vehicle.damage.gears")
    damageTracker.setDamage("powertrain", device.name, true)
  end

  device.gearRatio = device.gearRatios[device.gearIndex]
  if device.parent and device.parent.clutchRatio and device.parent.clutchRatio > 0.6 and device.gearIndex ~= 0 and avDifference > device.shiftDenialMinimumAVDifference then
    device.gearRatio = 0
...
local function setPartCondition(device, subSystem, odometer, integrity, visual)
  ...
  if type(integrity) == "number" then
    ...
    for gearIndex, _ in pairs(device.gearRatios) do
      integrityState.gearDamages[gearIndex] = 0
    end
  end

  device.damageFrictionCoef = integrityState.damageFrictionCoef or 1
  device.gearDamages = integrityState.gearDamages
...
local function getPartCondition(device)
  local integrityState = {
    ...
    gearDamages = device.gearDamages,
...
local function initSounds(device, jbeamData)
  device.gearGrindSoundFile = jbeamData.gearGrindSoundFile or "event:>Vehicle>Transmission>grind_legacy_01"
